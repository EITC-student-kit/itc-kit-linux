...
loc = string.format("%s/EITC-kit/Linux-version/ITCKit",os.getenv("HOME"))
-----------------------
...
function draw_db (cr, i)
	...
	local db = sqlite3.open(string.format("%s/db/itckitdb", loc))
...
function settings()
	...
	local settings = io.open(string.format("%s/settings/settingsFile", loc), "r")
	settings_array = {}
